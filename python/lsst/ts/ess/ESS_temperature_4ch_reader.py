...
# ----------------------------------------------------
def testVal(preamble, sensor_str):
    val = ERR_VAL
...
# ----------------------------------------------------
class EssTemperature4Ch:
    def __init__(self, uart=None, baudrate=19200, timeout=1.0):
    ...
    def readInstrument(self):
        ...
        try:
            ...
            print(
                ESS_INSTR_NAME,
                ": Received line failed decode to utf-8. Ignoring received line",
            )
        else:
...
class InstrumentThread(threading.Thread):
    def __init__(self, thread_id, instr):
