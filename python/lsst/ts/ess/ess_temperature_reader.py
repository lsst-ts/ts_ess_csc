...
class EssTemperature:
    ...
    def __init__(
        ...
        self._read_line_size: int = (self._channels * SENSOR_DATA_SIZE) - DELIM_SIZE
    ...
    def _message(self, text: Any) -> None:
        ...
        :rtype: None
        """
        logger.debug("EssTemperature:{}: {}".format(self._name, text))
        print("EssTemperature:{}: {}".format(self._name, text))
            ...
            if len(line) == self._read_line_size:
                ...
                for i in range(self._channels):
                    if self._test_val(self._preamble_str[i], temps[i]):
                        ...
                        err_flag = True
                if err_flag:
                    raise ValueError(self._name, ":", "Temperature data error.")
                else:
                    for i in range(self._channels):
                ...
                raise ValueError(
                    self._name, ":", "Serial timeout or received line length error."
                )
        else:
...
class InstrumentThread(threading.Thread):
    ...
    def terminate(self):
        ...
        :rtype: None
        """
        msg = "Stopping instrument read loop for:" + self._ess_instrument._name + "."
        self._message(msg)
        self.enabled = False
    ...
    def run(self):
        ...
        :rtype: None
        """
        msg = "Starting instrument read loop for:" + self._ess_instrument._name + "."
        self._message(msg)
        while self.enabled:
